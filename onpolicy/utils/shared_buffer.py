...
class SharedReplayBuffer(object):
        ...
        if len(self.share_obs.shape) > 4:
            ...
            if self.messages is not None:
                messages = (
                    self.messages[:-1]
                    .transpose(1, 2, 0, 3, 4, 5)
                    .reshape(-1, *self.messages.shape[3:])
                )
        else:
    ...
    def recurrent_group_generator(self, advantages, num_mini_batch, data_chunk_length):
        """
        Yield training data for chunked RNN training.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        ...
        """
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3]
        # Since we will sample the sequences randomly, this is just an estimate
        samples_per_rollout = episode_length // data_chunk_length
        mini_batch_size = (n_rollout_threads * samples_per_rollout) // num_mini_batch

        rollout_rand = torch.cat(
            [torch.randperm(n_rollout_threads) for _ in range(samples_per_rollout)]
        )
        rollout_sampler = [
            rollout_rand[i * mini_batch_size : (i + 1) * mini_batch_size]
            for i in range(num_mini_batch)
        ]
        # We subtract the data chunk length from the rollout length to ensure that every chunk is from only ine rollout and complete
        ts_rand = torch.randint(
            0, episode_length - data_chunk_length, (rollout_rand.numel(),)
        )
        ts_sampler = [
            ts_rand[i * mini_batch_size : (i + 1) * mini_batch_size]
            for i in range(num_mini_batch)
        ]
        ...
        for indices, timesteps in zip(rollout_sampler, ts_sampler):
            ...
            for idx, ts in zip(indices, timesteps):
                share_obs_batch.append(share_obs[ts : ts + data_chunk_length, idx])
                obs_batch.append(obs[ts : ts + data_chunk_length, idx])
                if self.messages is not None:
                    # We only need the first instance of the message, since they will be infered by the model
                    messages_batch.append(messages[ts, idx])
                actions_batch.append(actions[ts : ts + data_chunk_length, idx])
                if self.available_actions is not None:
                    available_actions_batch.append(
                        available_actions[ts : ts + data_chunk_length, idx]
                    )
                value_preds_batch.append(value_preds[ts : ts + data_chunk_length, idx])
                return_batch.append(returns[ts : ts + data_chunk_length, idx])
                masks_batch.append(masks[ts : ts + data_chunk_length, idx])
                active_masks_batch.append(
                    active_masks[ts : ts + data_chunk_length, idx]
                )
                old_action_log_probs_batch.append(
