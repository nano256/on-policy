...
CUDA_VISIBLE_DEVICES=0 python ../../train/train_mpe.py --env_name ${env} --user_name ${user_name} --use_wandb 0 \
    ...
    --model "ip" --algorithm_name "crmappo" --experiment_name "${exp_prefix}_seed_${seed}_comm_int_${communication_interval}_commit_coef_${commitment_coef}_n_agents_${num_agents}_n_landmarks_${num_landmarks}" --use_commitment_loss 1 \
