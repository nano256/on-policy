...
class SISLCommunicationRunner(Runner):
    """Runner class to perform training, evaluation. and data collection for the MPEs. See parent class for details."""

    def __init__(self, config):
        super(SISLCommunicationRunner, self).__init__(config)
    ...
    def pretrain_world_model(
        ...
        # We assume obs having the dims (Rollout, Agent, *Features)
        actions_per_step = np.product(obs.shape[:2])
        obs_predictor = policy.actor.observation_predictor
        cnn = policy.actor.cnn
        ...
        # Now we assume the dims (Rollout, Batch, Agent, Sequence, *Features)
        action_perm_seq = (0, 2, 3, 1, 4)
        obs_perm_seq = (0, 2, 3, 1, *np.arange(obs_batch.dim())[4:])
        obs_batch = obs_batch.permute(obs_perm_seq)
        action_batch = action_batch.permute(action_perm_seq)
        ...
        y_train = (
            obs_batch[:, :, :, :-1, ...]
            .reshape(-1, *obs_shape)[:n_samples]
            .to(**policy.actor.tpdv)
        )
        initial_obs_train = initial_obs.reshape(-1, *obs_shape)[:n_samples].to(
            **policy.actor.tpdv
        )
        actions_train = action_batch.reshape(-1, action_batch.size(-1))[:n_samples].to(
            **policy.actor.tpdv
        )
